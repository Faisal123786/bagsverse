...
const BankDetails = () => {
    ...
    // Bank Options
    const bankOptions = [
        "Bank Al Habib", "Meezan Bank", "HBL", "UBL", "Faysal Bank", "Allied Bank", "JazzCash", "EasyPaisa", "SadaPay", "Nayapay"
    ];
            ...
            <Row className="justify-content-center">
                ...
                <Col lg={4} className="mb-4 ">
                    <Card className="border-0 shadow-md bg-light">
                        <Card.Header className={`py-3 border-bottom ${isEditing ? 'bg-warning bg-opacity-10' : 'bg-light'}`}>
                            <h5 className="mb-0 fw-bold">{isEditing ? 'Edit Bank Account' : 'Add New Bank'}</h5>
                        </Card.Header>
                        <Card.Body className="p-4">
                            <Form onSubmit={handleSubmit}>
                                ...
                                <Form.Group className="mb-3">
                                    <Form.Label className="small fw-bold">Account Title</Form.Label>
                                    <Form.Control
                                        type="text"
                                        name="accountTitle"
                                ...
                                <Form.Group className="mb-3">
                                    <Form.Label className="small fw-bold">IBAN (Optional)</Form.Label>
                                    <Form.Control
                                        type="text"
                                        name="iban"
                                        placeholder="PK..."
                                ...
                                <div className="d-grid gap-2">
                                    <Button type="submit" className="btn-dark fw-bold" disabled={loading}>
                                ...
                                <tbody>
                                    {banks.length > 0 ? (
                                        banks.map((bank) => (
                                            <tr key={bank._id}>
                                                <td className="ps-4">
                                                    <span className="fw-bold text-dark">{bank.bankName}</span>
                                                </td>
                                                <td>
                                                    <div className="d-flex flex-column">
                                                        <span className="fw-bold text-dark">{bank.accountTitle}</span>
                                                        <span className="text-muted small">{bank.accountNumber}</span>
                                                        {bank.iban && <span className="text-muted small" style={{ fontSize: '0.75rem' }}>IBAN: {bank.iban}</span>}
                                                    </div>
                                                </td>
                                                <td>
                                                    {bank.isActive ?
                                                        <Badge bg="success" className="fw-normal">Active</Badge> :
                                                        <Badge bg="secondary" className="fw-normal">Hidden</Badge>
                                                    }
                                                </td>
