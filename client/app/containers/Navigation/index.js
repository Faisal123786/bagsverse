...
class Navigation extends React.PureComponent {
  ...
  componentWillUnmount() {
    window.removeEventListener("scroll", this.handleScroll); // 🔥 ADDED
  }

  handleScroll = () => {
    if (window.scrollY > 60) {
      if (!this.state.hideTopBar) this.setState({ hideTopBar: true });
    } else {
      if (this.state.hideTopBar) this.setState({ hideTopBar: false });
    }
  };
  ...
  render() {
    ...
    return (
      ...
      >
        <div
          className={`header-info ${this.state.hideTopBar ? "hide-bar" : ""}`}
        >
          <Container>
            <Row>
