...
export const productChange = (name, value) => {
  ...
  if (value instanceof FileList) {
    formData[name] = Array.from(value);
  } else {
    formData[name] = value;
  }
  ...
  return async (dispatch, getState) => {
    try {
      ...
      const newProduct = {
        ...
        image: product.image, // array of files
      ...
      for (const key in newProduct) {
        if (!newProduct.hasOwnProperty(key)) continue;
        ...
        if (key === "image" && Array.isArray(value)) {
          // append all images
          value.forEach((file) => {
            formData.append("image[]", file); // note the 'image[]' for multiple files
          });
        } else {
